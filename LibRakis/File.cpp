...
void File::init()
{
	...
	name_.assign(it, path_.end());
}

const wstring File::defaultPrefix(L"temp_");
const wstring File::defaultExt(L"tmp");

File File::tempFile(wstring const& prefix, wstring const& ext) const
{
    random_generator gen;
    uuid id = gen();
    wostringstream newPath;
    newPath << path_.c_str() << separator << prefix << id << "." << ext;
    return File(newPath.str());
}
